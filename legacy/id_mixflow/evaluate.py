...
def evaluate_sasrec(
    ...
    for batch_start in range(0, len(users), batch_size):
        ...
        for user in batch_users:
            seq = dataset.user_seq[user]
            # For both val/test: predict the last item, using all previous as context.
            if len(seq) < 2:
...
def run_eval_online_fme(
    ...
    patch_detached = patch.detach() if patch is not None else None

    def _maybe_select_patch(batch: dict, dev: torch.device, mask: Optional[torch.Tensor]) -> Optional[torch.Tensor]:
        if patch_fn is None:
            return patch_detached
        out = patch_fn(batch, dev, mask)
        return out.detach() if out is not None else None
