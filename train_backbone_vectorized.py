...
def train_sasrec(
    ...
    logger.info("Sampling strategy: vectorized random negatives (1 per position) with DataLoader")
...
if __name__ == "__main__":
    ...
    run_name = (
        ...
        f"{config.hidden_units}-{config.max_seq_length}_{es_suffix}-vectorized"
    )
    run = wandb.init(project=f"backbone-standard-{config.dataset}", name=run_name, config=config.__dict__)
    base_ckpt_dir = config.checkpoint_dir / f"{config.backbone}_loo_vectorized"
