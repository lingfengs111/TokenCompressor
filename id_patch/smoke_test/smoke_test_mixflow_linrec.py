"""MixFlow 2nd-order gradient smoke test for LinRec backbone."""
...
class TinyConfig:
    ...
    linrec_num_layers: int = 2
    linrec_num_heads: int = 1
    linrec_inner_size: int = 64
    linrec_hidden_dropout: float = 0.1
    linrec_attn_dropout: float = 0.1
    linrec_hidden_act: str = "gelu"
...
def _select_theta_names(model: torch.nn.Module, enable_bias_ln: bool = True, enable_head: bool = True) -> List[str]:
    trainable = set()
    if enable_bias_ln:
...
def main():
    ...
    config = TinyConfig()
    item_num = 50
    model = LinRec(config, item_num=item_num).to(device)
