"""MixFlow 2nd-order gradient smoke test for FMLP backbone."""
...
class TinyConfig:
    ...
    # FMLP specific
    fmlp_num_layers: int = 2
    fmlp_num_heads: int = 1
    fmlp_hidden_dropout: float = 0.1
    fmlp_attn_dropout: float = 0.1
    fmlp_hidden_act: str = "gelu"
...
def main():
    ...
    config = TinyConfig()
    item_num = 50
    model = FMLP(config, item_num=item_num).to(device)
