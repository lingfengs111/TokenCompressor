...
class IDTrainConfig:
    ...
    def log_config(self):
        ...
        logger.info(
            "device: %s | num_epochs: %s | lr: %s | wd: %s",
            self.device,
            self.num_epochs,
...
class SequentialSampler:
    ...
    def __init__(self, dataset: SequenceDataset, batch_size: int, max_seq_length: int, train_stride: int = 1):
        ...
        for user in dataset.users:
            ...
            if len(seq) > 2:
                # Exclude the last two items for validation/test targets
    ...
    def __iter__(self):
        ...
        for i in range(0, len(indices), self.batch_size):
            ...
            user_ids = torch.tensor([u for u, _ in batch_data], dtype=torch.long)
